...
public class StartSessionDialogController : MonoBehaviour
{
    ...
    private HashSet<RemoteObject> _objects = new HashSet<RemoteObject>();//HashSet<T> クラス

    #region MonoBehavior Functions
    /// <summary>
    /// Delay showing the dialog slightly.
    /// </summary>
    private async void Start()//初回update関数読み込み前の1回だけ実行（awake→start→update）
    {
    ...
    {
        yield return 0;//yield return null とほぼ同じ
    ...
    private void FindAndRemoveFromObjectList(RemoteObject remoteObject)
    {
        ...
        {
            _objects.Remove(remoteObject);//HashSet<T> オブジェクトから指定された要素を削除します。
    ...
    private void RemoveAllFromObjectList()
    {
        ...
        _objects.Clear();//HashSet<T> オブジェクトからすべての要素を削除します。
    }
